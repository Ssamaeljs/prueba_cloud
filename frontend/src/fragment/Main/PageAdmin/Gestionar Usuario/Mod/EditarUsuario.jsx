...
const EditarUsuario = (props) => {
  ...
  useEffect(() => {
    ...
    setValue("fecha_nacimiento", usuario.fecha_nacimiento);
    if (usuario.cuenta.rol === "ADMINISTRADOR") {
      setValue("institucion", "Universidad Nacional de Loja");
      setValue("cargo", "Administrativo");
      setShowCargo(true);
    } else {
    ...
    setValue("rol", usuario.cuenta.rol);
    setValue("correo", usuario.cuenta.correo);
  }, [usuario, setValue]);
  ...
  const onSubmit = (data) => {
    if (paginaActual === 1) {
      ...
      POST(datos, `guardar/persona/${usuario.external_id}`, getToken())
        .then((info) => {
          if (info.code !== 200) {
            mensajes("Hubo un error en el registro", "error", "Error");
          } else {
            mensajes("Usuario editado exitosamente", "success", "Éxito");
            setShow(false);
            setTimeout(() => {
              ...
              <div className="row">
                ...
                <div className="text-center mt-5 mb-2">
                  ...
                  <Button
                    ...
                    disabled={isSubmitting}
                  >
                    {paginaActual === 1 ? "Siguiente" : "Editar"}
                  </Button>
                </div>
              </div>
            </form>
